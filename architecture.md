...
- **GPU-Resident Exact Pass:** For the final, full-resolution pass (Pass 3), the grayscale image and template are uploaded to the GPU once. The search is then performed entirely on the device by `golden_section_scale_search_gpu`, which repeatedly calls `eval_scale_gpu`. This avoids costly host-device memory transfers inside the search loop, resolving a major performance bottleneck.
